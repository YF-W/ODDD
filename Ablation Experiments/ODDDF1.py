...
def Model(X, max_k, omega, eta, mu, niu, contamination):
    ...
    for i in range(1, mu+1):  # 4. 外层循环，遍历大步进
        ...
        outlier_score_list,y_predict_type=Assessment(point_list, contamination)
...
# 小移动
def Interference_Detection_Module(point_list, previous_point_list, max_k, eta, theta):
    """
...
#-----right-----------
    ...
    if left_score >= right_score:  # 如果左移动的孤立点得分大于等于右移动的孤立点得分
        ...
        return sorted_bright_list_id_left, left_score, outlier_list_left, part_outlier_list_left,point_list
        # 返回排序好的亮度列表，点的得分，左移动的离群值，左移动的部分异常值
    else:
        point_list=copy.copy(point_list_right)
        return sorted_bright_list_id_right, right_score, outlier_list_right, part_outlier_list_right,point_list  # 返回排序好的亮度列表，点的得分，右移动的离群值，右移动的部分异常值
...
def Assessment(point_list, contamination):
    ...
    for i in range(0, len(point_list)):
        ...
        point_list[i].outlier_degree = point_list[i].move_distance/point_list[i].bright  # 孤立度=移动距离/能量
    ...
    #
    # # 更新最大孤立度
    # for i in range(1, len(sorted_outlier_list) - 1):
    #     cut_list1 = sorted_outlier_list[:i]
    #     cut_list2 = sorted_outlier_list[i:]
    #     score=anomaly_score(cut_list1,cut_list2)
    #     if score>max_score:
    #         max_score=score
    #         cut_point=i

    return outlier_list,y_predict_type   # 返回分数，即评估当前的移动好坏
