...
def Model(X, max_k, omega, eta, mu, niu, contamination):
    ...
    for i in range(1, mu+1):  # 4. 外层循环，遍历大步进
        ...
        for j in range(1, niu+1):  # 6. 内层循环，扰动探测，寻找周围的最佳解
            sorted_bright_list, outlier_score, outlier_score_list, part_outlier_list,point_list = Interference_Detection_Module(
                point_list, previous_point_list, max_k, eta, theta)
            # 输出离群值评分的列表
...
def Data_Association_Module(point_list, neighbor_matrix, max_k, type):
    ...
    for i in range(0, len(point_list)):
        ...
        # TODO 连接性正负数
        # 通过连接性算亮度
        if point_list[i].outdegree==0:  # 如果是小扰动
            point_list[i].bright = 1  # 求小扰动的亮度

        else:  # 其它情况就是公式二
            point_list[i].bright = sigmoid(point_list[i].density)  # 直接计算点的亮度，点的连接性的sigmoid值
    bright_list = []  # 首先创建一个亮度的空列表
    # 7. 确定休眠点；
    for i in range(0, len(point_list)):
        bright_list.append(point_list[i].bright)
        if point_list[i].bright ==1:  # 比较点亮度是否大于临界值
